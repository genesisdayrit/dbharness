...
`dbharness snapshot` creates a timestamped backup of your `.dbharness/` directory. Snapshots are saved to a `.dbharness-snapshots/` folder in the current directory.

## Snapshot everything

Running `dbharness snapshot` copies the entire `.dbharness/` directory:
...
Running `dbharness snapshot config` copies only the `config.json` file:
...
- A `.dbharness/` directory must exist before snapshotting. Run `dbharness init` first if you haven't already.
